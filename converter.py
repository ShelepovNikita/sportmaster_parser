"""Модуль отвечает за конвертацию полученных данных в
форматы csv, xml, json, excel.
"""
import csv
import os
...
def to_xml(
        ...
        parameters: Optional[str | list[str]] = '',
        indent: Optional[str] = '\t',
        root: Optional[str] = 'items',
...
def to_json(data: Iterable[Mapping[str, Any]],
    ...
    """
    datetime_now = get_datetime()
    with open(f'resulting files/{file_name} {datetime_now}.json',
...
def to_csv(
    ...
    """
    datetime_now = get_datetime()
    with open(f'resulting files/{file_name} {datetime_now}.csv',
